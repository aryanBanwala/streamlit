"""
Waitlist Review - Review waitlist users and mark them for removal
"""
...
if 'waitlist_current_page' not in st.session_state:
    st.session_state.waitlist_current_page = 1
if 'waitlist_users_per_page' not in st.session_state:
    st.session_state.waitlist_users_per_page = 10
...
st.session_state.waitlist_users_per_page = users_per_page

total_filtered = len(filtered_users)
total_pages = max(1, (total_filtered + users_per_page - 1) // users_per_page)

# Ensure current page is within bounds
if st.session_state.waitlist_current_page > total_pages:
    st.session_state.waitlist_current_page = total_pages
...
st.sidebar.number_input(
    ...
    value=st.session_state.waitlist_current_page,
    key='waitlist_page_input',
    on_change=lambda: setattr(st.session_state, 'waitlist_current_page', st.session_state.waitlist_page_input)
)
...
with col_results:
    st.subheader(f"Showing {total_filtered} users (Page {st.session_state.waitlist_current_page}/{total_pages})")

# Calculate slice for current page
start_idx = (st.session_state.waitlist_current_page - 1) * users_per_page
...
with col_prev:
    if st.button("< Previous", disabled=(st.session_state.waitlist_current_page <= 1), use_container_width=True):
        st.session_state.waitlist_current_page -= 1
        st.rerun()

with col_info:
    st.markdown(f"<center>Page {st.session_state.waitlist_current_page} of {total_pages}</center>", unsafe_allow_html=True)

with col_next:
    if st.button("Next >", disabled=(st.session_state.waitlist_current_page >= total_pages), use_container_width=True):
        st.session_state.waitlist_current_page += 1
        st.rerun()
