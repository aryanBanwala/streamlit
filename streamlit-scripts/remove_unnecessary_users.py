...
# Configuration
st.set_page_config(page_title="Remove Unnecessary Users", layout="wide")
    ...
    for idx, removed_user in enumerate(removed_users):
        ...
        if user_index is not None:
            if st.sidebar.button(f"{name} - removed", key=f"removed_{idx}", use_container_width=True):
                st.session_state.current_index = user_index
                st.rerun()
        else:
            st.sidebar.markdown(
...
if st.session_state.current_index >= len(users):
    st.session_state.current_index = len(users) - 1
if st.session_state.current_index < 0:
    st.session_state.current_index = 0

current_user = users[st.session_state.current_index]
user_id = current_user.get('user_id')
is_removed = current_user.get('shouldBeRemoved') == True

# Progress indicator
st.markdown(f"**User {st.session_state.current_index + 1} of {len(users)}**")
st.progress((st.session_state.current_index + 1) / len(users))
...
with col_prev:
    if st.button("< Previous", key="prev_btn", use_container_width=True, disabled=(st.session_state.current_index == 0)):
        st.session_state.current_index -= 1
        st.rerun()

with col_next:
    if st.button("Next >", key="next_btn", use_container_width=True, disabled=(st.session_state.current_index >= len(users) - 1)):
        st.session_state.current_index += 1
        st.rerun()

# Footer
st.divider()
st.caption("Remove Unnecessary Users - Review and mark users for removal")
